...
import { BRAND, CONTACT, OPERATOR, REGISTERED_ADDRESS, waLink } from "@/src/config/site";
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-9 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <div className="mb-5 flex flex-col gap-3">
              {/* Home SOP §17: phone, GSTIN, registered address and email on
                  every page. The address falls back to the service locality
                  until the registered address is confirmed in site config. */}
              {[
                { Icon: Phone, text: CONTACT.phoneDisplay, href: `tel:${CONTACT.phone}` },
                { Icon: MapPin, text: REGISTERED_ADDRESS || "Dwarka, Saurashtra, Gujarat — 361335" },
                ...
                <div key={text} className="flex items-start gap-3">
                  ...
                  {href ? (
                    <a href={href} className="mt-1 text-[13px] leading-snug text-gray-600 hover:text-orange-600">
                      {text}
                    </a>
                  ) : (
                    <span className="mt-1 text-[13px] leading-snug text-gray-600">{text}</span>
                  )}
                </div>
              ))}
              <p className="text-[12.5px] leading-snug text-gray-500">
                GSTIN: <span className="font-semibold text-gray-700">{OPERATOR.gstin}</span>
              </p>
            </div>
      ...
      <div className="border-t border-orange-100/80">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-5 py-4 sm:flex-row md:px-10">
          <p className="text-center text-[12.5px] text-gray-500 sm:text-left">
            © {new Date().getFullYear()} {BRAND.name} · Operated by {OPERATOR.parent} · All rights reserved
            {" · "}
            Made by{" "}
            <a
