...
/**
 * The shared hero for a package hub: warm wash, animated temple silhouette,
 * rising diyas and the stats strip, ending in the wave into white.
 *
 * The markup originated on /somnath-dwarka-tour-package/ and is identical
 * across hubs apart from the four things that are props here — headline,
 * description, the mobile photo and the stats. Hubs pass their own copy rather
 * than each carrying a 300-line copy of the same SVG.
 *
 * `headline` and `description` take nodes, not strings, because both carry
 * inline markup: the headline highlights a phrase with a gradient span, and the
 * description bolds the temple names.
 */
export type HubHeroStat = { value: string; label: string };

export default function PackageHubHero({
  headline,
  description,
  mobileImage,
  stats,
  waMessage,
  browseHref = "#by-duration",
}: {
  headline: ReactNode;
  description: ReactNode;
  mobileImage: { src: string; alt: string; title: string; region: string };
  stats: HubHeroStat[];
  waMessage: string;
  browseHref?: string;
}) {
  return (
    <section id="hero" className="font-dm relative -mt-28 flex flex-col overflow-hidden">
      ...
      <div className="absolute inset-0 z-0" aria-hidden="true">
        ...
        <div className="absolute -right-[6%] top-[20%] h-[46%] w-[36%] bg-[radial-gradient(ellipse_at_center,rgba(132,204,22,0.12)_0%,transparent_64%)]" />
        <div
        ...
        <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <div className="pkg-anim pkg-d1 relative mt-2 lg:hidden w-full flex justify-center">
            <div className="relative w-full max-w-[440px] [aspect-ratio:16/11] overflow-hidden rounded-[26px] border-4 border-white shadow-[0_20px_50px_rgba(234,88,12,0.25)]">
              <img src={mobileImage.src} alt={mobileImage.alt} className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 px-4 pb-3.5 pt-8 text-left text-white">
                <p className="font-playfair text-base font-bold">{mobileImage.title}</p>
                <p className="mt-0.5 flex items-center gap-1.5 text-[11px] text-white/85"><MapPin size={10} /> {mobileImage.region}</p>
              </div>
            </div>
          </div>
          ...
          <div className="pkg-anim pkg-d4 mt-6 sm:mt-12 flex flex-row items-center justify-between gap-2 rounded-2xl border border-orange-200/40 bg-white/45 p-3 sm:p-4 shadow-sm backdrop-blur-xs w-full sm:w-auto">
            {stats.map((stat, i) => (
              <Fragment key={stat.label}>
                {i > 0 ? <div className="w-px h-8 sm:h-10 bg-orange-200/50 shrink-0" /> : null}
                <div className="flex-1 text-center px-2">
                  <div className="font-playfair text-xl font-black leading-none text-orange-600 sm:text-3xl">
                    {stat.value}
                  </div>
                  <div className="mt-1.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-[0.08em] text-[#8c5e40] whitespace-nowrap">
                    {stat.label}
                  </div>
                </div>
              </Fragment>
        ...
        <div className="relative hidden h-[500px] w-full items-center justify-center lg:flex overflow-visible">
          ...
          >
            ...
            <g fill="url(#temple-outline-grad)" stroke="url(#temple-stroke-grad)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round">
              ...
              {/* Central shikhara with vertical ribs */}
